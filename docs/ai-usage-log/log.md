...
| 2569-08-17 | Claude Code | Sprint 3 (ปิด session + รายงานสรุปมา/ขาด) | ปุ่มปิดคลาสเช็คอิน (`closeSession`/`closeSessionAction`), หน้า `/teacher/reports` + `[sessionId]` แสดงตารางมา/ขาดกรองตามกลุ่มเรียน, nav ในหน้าอาจารย์ | ยืนยันแพลนก่อนเริ่ม (ปิด session เป็น prerequisite ของ UC3), ทดสอบครบกับ Sheet จริง (ปิด session, บล็อกเช็คชื่อซ้ำ, สลับ session ในรายงาน) |
| 2569-08-17 | Claude Code | เพิ่ม feature export รายงานเป็น .xlsx | Route Handler `/teacher/reports/[sessionId]/export`, ปุ่ม Export ในหน้ารายงาน | พบว่า `xlsx` บน npm มี CVE ไม่มี patch เปลี่ยนไปติดตั้งจาก CDN ของ SheetJS เอง แทน, ทดสอบด้วยการ generate + round-trip อ่านไฟล์จริงกลับมาเทียบกับ Sheet |
| 2569-08-17 | Claude Code | เพิ่ม export สรุปทั้งเทอมต่อกลุ่มเรียน (ตาราง matrix) | Route Handler `/teacher/reports/[sessionId]/export-summary`, `getAttendanceBySession()` อ่าน log ครั้งเดียวแทนวนอ่านต่อ session, ปุ่ม export ที่สอง | ถามรูปแบบก่อนทำ (matrix vs สรุปเฉยๆ) ผู้ใช้เลือก matrix, ทดสอบกับ N01 (6 sessions) และ N03 (มีคนเช็คชื่อจริง) เทียบ Sheet ตรงทุกจุด |
