...
- **ขยาย tolerance การยืนยัน token เป็น 180 วินาที (2569-08-16, หลังทดสอบจริง)**
  ...
  **trade-off ที่ต้องรับทราบ**: QR ที่ถูกถ่ายรูป/แคปหน้าจอตอนนี้ใช้เช็คชื่อ
  ...
  เดิมจาก spec
- **(A)→(B) เปลี่ยนใจ: ผูกกลุ่มเรียนกับนักศึกษาแล้ว (2569-08-16)** — ตอนแรก
  เลือก (A) ไม่ผูกกลุ่ม (ยอมรับ trade-off ตาม sprint-1.md) แต่อาจารย์เพิ่ม
  คอลัมน์ **`Section`** (คอลัมน์ C) ลง sheet `students` เองด้วยมือ (71 แถว
  มีค่าครบทุกแถว: N01=33, N02=31, N03=7) จึงอัปเดต schema ใน
  data-dictionary.md และเพิ่มการตรวจใน `checkinAction`: หลังพบรหัส
  นักศึกษาใน master list แล้ว เทียบ `student.section` กับ section ที่
  encode ใน `session_id` (`lib/session.ts: parseSessionId`) — ไม่ตรงกัน
  → error "รหัสนักศึกษานี้อยู่กลุ่มเรียน X ไม่ใช่กลุ่ม Y ของ session นี้"
  ทดสอบแล้วทั้งสองทาง (กลุ่มตรง → ผ่าน, กลุ่มไม่ตรง → ถูกปฏิเสธ) กับ
  Sheet จริง
